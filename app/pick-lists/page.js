...
export default function PickListDetailPage() {
  ...
  async function loadData() {
    ...
    const [plRes, itemsRes, prodRes, userRes] = await Promise.all([
      supabase.from('pick_lists').select('id, status, approval_status, rejection_reason, do_no, customers ( name )').eq('id', id).single(),
      supabase
        .from('pick_list_items')
        .select(`
          id, requested_qty, picked_qty,
          stock_items (
            id, lot_no, expiry_date, qty, unit,
    ...
    if (userId) {
      const { data: profile } = await supabase.from('profiles').select('role_id, roles ( permissions )').eq('id', userId).single()
      setCanApprove(!!profile?.roles?.permissions?.['pick_list.approve'])
    }
  ...
  async function handleReject() {
    if (!rejectReasonInput) { alert('กรุณาระบุเหตุผลที่ปฏิเสธ'); return }
    setApproving(true)
    const { error } = await supabase.from('pick_lists').update({
      approval_status: 'rejected', rejection_reason: rejectReasonInput
    }).eq('id', id)
    setApproving(false)
    if (error) { alert('ปฏิเสธไม่สำเร็จ: ' + error.message); return }
    setShowRejectBox(false); setRejectReasonInput('')
    loadData()
  }

  async function handleResubmit() {
    await supabase.from('pick_lists').update({ approval_status: 'pending', rejection_reason: null }).eq('id', id)
    loadData()
  }
  ...
  const approval = APPROVAL_LABEL[pickList.approval_status] ?? { label: pickList.approval_status, tone: 'warning' }
  const canShip = pickList.approval_status === 'approved'
      ...
      <main className="main">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
          <div>
            ...
            {' '}
            <span className={`badge ${approval.tone}`}>{approval.label}</span>
            {pickList.do_no && <span className="badge success" style={{ marginLeft: 6 }}>DO: {pickList.do_no}</span>}
          </div>
          {isEditable && (
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={handleCancel}>ยกเลิกใบ</button>
              <button className="primary" onClick={handleConfirmShipment} disabled={confirming || !canShip} title={!canShip ? 'ต้องรออนุมัติก่อน' : ''}>
        ...
        {pickList.do_no && (
          <p style={{ marginTop: 8 }}>
            <a href={`/delivery-orders/${encodeURIComponent(pickList.do_no)}`} style={{ fontSize: 13 }}>ดู Delivery Order →</a>
          </p>
        )}

        {pickList.approval_status === 'rejected' && (
          <div className="stat-card" style={{ marginTop: 12, borderColor: 'var(--danger-ink)' }}>
            <strong style={{ fontSize: 13, color: 'var(--danger-ink)' }}>ถูกปฏิเสธ</strong>
            <p style={{ fontSize: 13, margin: '6px 0' }}>{pickList.rejection_reason}</p>
            {isEditable && <button onClick={handleResubmit}>ส่งขออนุมัติใหม่</button>}
          </div>
        )}

        {canApprove && pickList.approval_status === 'pending' && isEditable && (
          <div className="stat-card" style={{ marginTop: 12, background: 'var(--warning-bg)' }}>
            <strong style={{ fontSize: 13 }}>รออนุมัติจากคุณ</strong>
            <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
              <button className="primary" onClick={handleApprove} disabled={approving}>{approving ? 'กำลังบันทึก...' : 'อนุมัติ'}</button>
              <button onClick={() => setShowRejectBox((v) => !v)}>ปฏิเสธ</button>
            </div>
            {showRejectBox && (
              <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                <input placeholder="เหตุผลที่ปฏิเสธ" value={rejectReasonInput} onChange={(e) => setRejectReasonInput(e.target.value)} style={{ flex: 1 }} />
                <button onClick={handleReject} disabled={approving}>ยืนยันปฏิเสธ</button>
              </div>
            )}
          </div>
        )}
          ...
          <tbody>
            {items.map((it) => (
              <tr key={it.id}>
                ...
                <td className="mono">{it.requested_qty} {it.stock_items?.unit || it.stock_items?.inventory_items?.unit || ''}</td>
                <td>
                  {isEditable ? (
                    <input
                      type="number"
